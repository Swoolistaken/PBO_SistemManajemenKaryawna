...
/**
 * Main Frame Aplikasi - Entry point GUI
 * Implementasi: GUI SWING, MULTITHREAD (SwingUtilities.invokeLater)
 */
public class Main extends JFrame {
    ...
    private void initLookAndFeel() {
        try {
            ...
            UIManager.put("Table.alternateRowColor", new Color(38, 41, 54));
    ...
    private void initPanels() {
        ...
        viewData.setCallback(new ViewData.ActionCallback() {
            ...
            @Override public void onEdit(ModelKaryawan k) {
            ...
            @Override public void onLihatKPI(ModelKaryawan k) {
                tampilkan("KPI");
                editData.refreshKaryawan();
            }
            @Override public void onLihatAbsensi(ModelKaryawan k) {
                tampilkan("ABSENSI");
            }
        });
    ...
    private JButton buatNavBtn(String teks, String panel) {
        ...
        btn.addMouseListener(new MouseAdapter() {
            @Override public void mouseEntered(MouseEvent e) {
                if (btn != btnAktif) btn.setBackground(BG_NAV_HOVER);
            }
            @Override public void mouseExited(MouseEvent e) {
                if (btn != btnAktif) btn.setBackground(BG_NAV);
            }
        });
        ...
        btn.addActionListener(e -> {
            ...
            if ("KARYAWAN".equals(panel)) viewData.loadData();
            if ("KPI".equals(panel)) editData.refreshKaryawan();
            if ("ABSENSI".equals(panel)) viewAbsensi.refreshKaryawan();
        });
    ...
    private JPanel buatPanelGaji() {
        ...
        btnKalkulasi.setContentAreaFilled(true);
        btnKalkulasi.setBorder(new EmptyBorder(10, 20, 10, 20));
        btnKalkulasi.setCursor(new Cursor(Cursor.HAND_CURSOR));
        btnKalkulasi.addMouseListener(new MouseAdapter() {
            @Override public void mouseEntered(MouseEvent e) { btnKalkulasi.setBackground(new Color(20, 75, 150)); }
            @Override public void mouseExited(MouseEvent e)  { btnKalkulasi.setBackground(new Color(34, 100, 180)); }
        });
        btnKalkulasi.setPreferredSize(new Dimension(250, 44));
        btnKalkulasi.addActionListener(e -> kalkulasiGaji());

        JPanel center = new JPanel(new GridBagLayout());
        center.setBackground(new Color(30, 33, 43));
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.gridy = 0; center.add(lbl, gbc);
